...
export class StorageService {
  ...
  async uploadObject(input: StorageUploadInput): Promise<StorageObjectResult> {
    if (!this.client) {
      this.fallbackObjects.set(
        this.getFallbackMapKey(input.bucket, input.key),
        input.body,
      );
      return {
        bucket: input.bucket,
  ...
  private async ensureBucketExists(bucket: string): Promise<void> {
    ...
    try {
      ...
      this.logger.log(
        `Created storage bucket \`${bucket}\` for local media uploads.`,
      );
      this.verifiedBuckets.add(bucket);
    } catch (error) {
      const message = error instanceof Error ? error.message : 'unknown error';
      ...
      if (
        /already exists|bucketalreadyownedbyyou|bucketalreadyexists/i.test(
          message,
        )
      ) {
        this.verifiedBuckets.add(bucket);
