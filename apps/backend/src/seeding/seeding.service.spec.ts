...
import { AuthProvider, CouponType, ProductStatus, Role } from '@prisma/client';
  ...
  return {
    ...
    user: {
      ...
      upsert: async ({
        ...
        where: { email: string };
        create: Partial<UserRecord> & { email: string; role: Role };
        ...
        const record: UserRecord = existing
          ...
          : {
              ...
              emailVerifiedAt: create.emailVerifiedAt ?? null,
              createdAt: now,
              updatedAt: now,
            };
    ...
    authIdentity: {
      findUnique: async ({
        ...
        identities.get(
          `${where.provider_providerSubject.provider}:${where.provider_providerSubject.providerSubject}`,
        ) ?? null,
      upsert: async ({
        where,
        create,
        ...
        create: Omit<AuthIdentityRecord, 'createdAt' | 'updatedAt'>;
        update: Partial<AuthIdentityRecord>;
      }) => {
    ...
    category: {
      findUnique: async ({ where }: { where: { slug: string } }) =>
        categories.get(where.slug) ?? null,
      upsert: async ({
        where,
        create,
        update,
    ...
    tag: {
      ...
      upsert: async ({
        ...
        where: { slug: string };
        create: Omit<TagRecord, 'id'>;
        update: Partial<TagRecord>;
      }) => {
        const existing = tags.get(where.slug);
        const record: TagRecord = existing
          ? { ...existing, ...update }
          : { id: nextId('tag'), ...create };
    ...
    product: {
      ...
      upsert: async ({
        ...
        where: { slug: string };
        create: Omit<ProductRecord, 'id' | 'tagSlugs'> & {
          tags?: { connect: Array<{ slug: string }> };
        };
        update: Partial<ProductRecord> & {
          tags?: { set?: []; connect?: Array<{ slug: string }> };
        };
      }) => {
        ...
        const record: ProductRecord = existing
          ? {
              ...
              ...update,
              tagSlugs:
                update.tags?.connect?.map((tag) => tag.slug) ??
                existing.tagSlugs,
            }
          : {
              id: nextId('product'),
      ...
      update: async ({
        ...
        where: { slug: string };
        data: { tags?: { set?: []; connect?: Array<{ slug: string }> } };
      }) => {
        const existing = products.get(where.slug);
        if (!existing) {
    ...
    productVariant: {
      ...
      upsert: async ({
        ...
        where: { sku: string };
        create: Omit<ProductVariantRecord, 'id'>;
        update: Partial<ProductVariantRecord>;
      }) => {
    ...
    productVariantPrice: {
      findUnique: async ({
        ...
        where: {
          variantId_currencyCode: { variantId: string; currencyCode: string };
        };
      }) =>
        prices.get(
          `${where.variantId_currencyCode.variantId}:${where.variantId_currencyCode.currencyCode}`,
        ) ?? null,
      upsert: async ({
        where,
        create,
        ...
        create: Omit<ProductPriceRecord, 'id'>;
        update: Partial<ProductPriceRecord>;
      }) => {
    ...
    coupon: {
      ...
      upsert: async ({
        ...
        where: { code: string };
        create: Omit<CouponRecord, 'id' | 'createdAt' | 'updatedAt'>;
        update: Partial<CouponRecord>;
      }) => {
